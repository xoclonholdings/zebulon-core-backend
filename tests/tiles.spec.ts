...
async function step<T>(name: string, fn: () => Promise<T>): Promise<T> {
  try { return await test.step(name, fn); }
  catch (err: any) { throw new Error(`[${name}] ${err?.message ?? err}`); }
}
...
test.describe('Zebulon Homeview E2E', () => {
  test('home + tiles smoke', async ({ page, request }) => {
    ...
    await step('Open Home', async () => {
      ...
      expect(resp?.status(), 'Home did not load (HTTP error). Check dev server, E2E_BASE_URL, or Vite preview.').toBe(200);
      await expect(page.locator(SEL.appRoot)).toBeVisible({ timeout: 15000 });
      expect(await page.$(SEL.appRoot), 'Home did not load (no #app root). Check dev server, E2E_BASE_URL, or Vite preview.').not.toBeNull();
      expect(consoleErrors, 'Console errors on home load. Check browser console for stack.').toEqual([]);
    });

    await step('Verify grid + tiles visible', async () => {
      await expect(page.locator(SEL.grid)).toBeVisible({ timeout: 15000 });
      for (const [key, sel] of Object.entries(SEL.tile)) {
        expect(await page.isVisible(sel), `Tile ${key.toUpperCase()} not visible. Check data-testid and grid render.`).toBeTruthy();
      }
    });

    // Helper for tile navigation
    async function openTile(tile: keyof typeof SEL.tile, readySel: string, expectedPath: string, readyMsg: string, action: () => Promise<void>) {
      await step(`Open ${tile.toUpperCase()}`, async () => {
        await page.click(SEL.tile[tile]);
        await page.waitForURL(expectedPath, { timeout: 10000 });
        expect(page.url(), `${tile.toUpperCase()} tile click did not navigate to ${expectedPath}. Ensure router path and Link are wired.`).toContain(expectedPath);
        await page.waitForSelector(readySel, { timeout: 10000 });
        expect(await page.isVisible(readySel), `${tile.toUpperCase()} view ready marker not found. Ensure component mounts and data-testid='${tile}-ready' exists.`).toBeTruthy();
        await action();
        // Return home
    ...
    await openTile('zed', SEL.ready.zed, '/zed', 'ZED ready marker', async () => {
      if (await page.$('[data-testid="zed-input"]')) {
        ...
        expect(await page.inputValue('[data-testid="zed-input"]'), 'ZED input not accepting text.').toBe('test');
      }
    });
    await openTile('zeta', SEL.ready.zeta, '/zeta', 'ZETA ready marker', async () => {
      if (await page.$('[data-testid="firewall-toggle"]')) {
        ...
        expect(true, 'ZETA firewall toggle not clickable.').toBeTruthy();
      }
    });
    await openTile('zlab', SEL.ready.zlab, '/zlab', 'ZLab ready marker', async () => {
      if (await page.$('[data-testid="zlab-kanban"]')) {
        expect(await page.isVisible('[data-testid="zlab-kanban"]'), 'ZLab Kanban board not visible.').toBeTruthy();
      }
    });
    await openTile('zwap', SEL.ready.zwap, '/zwap', 'ZWAP ready marker', async () => {
      if (await page.$('[data-testid="wallet-status"]')) {
        expect(await page.isVisible('[data-testid="wallet-status"]'), 'ZWAP wallet badge not visible.').toBeTruthy();
      }
    });
    await openTile('zync', SEL.ready.zync, '/zync', 'ZYNC ready marker', async () => {
      if (await page.$('[data-testid="zync-editor"]')) {
        expect(await page.isVisible('[data-testid="zync-editor"]'), 'ZYNC IDE pane not visible.').toBeTruthy();
      }
    });
    await openTile('zulu', SEL.ready.zulu, '/zulu', 'ZULU ready marker', async () => {
      if (await page.$('[data-testid="zulu-diagnostics"]')) {
        expect(await page.isVisible('[data-testid="zulu-diagnostics"]'), 'ZULU diagnostics list not visible.').toBeTruthy();
      }
    });
    ...
    await step('Open Core panel', async () => {
      ...
      expect(await page.isVisible(SEL.views.core), 'Core panel not visible. Check data-testid="core-panel".').toBeTruthy();
    });
    await step('Open Settings', async () => {
      ...
      expect(await page.isVisible(SEL.views.settings), 'Settings panel not visible. Check data-testid="settings-panel".').toBeTruthy();
    });

    if (API_BASE) {
      await step('API health', async () => {
        const resp = await request.get(`${API_BASE}/health`);
        expect(resp.status(), 'API health endpoint did not return 200. Check backend Railway/port/env.').toBe(200);
        const json = await resp.json();
        expect(json.ok, 'API health endpoint did not return { ok: true }. Check backend health logic.').toBeTruthy();
      });
    }
  });
});
