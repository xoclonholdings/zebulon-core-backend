    ...
    return (
      <div style={{ 
        ...
        <div style={{ marginTop: '20px', padding: '10px', border: '1px solid #333', background: '#222' }}>
          <p style={{ color: 'red', fontWeight: 'bold' }}>Error: {error?.message || 'Unknown error'}</p>
          {error?.stack && (
            <pre style={{ color: '#ffb', fontSize: '0.9em', marginTop: '10px', whiteSpace: 'pre-wrap' }}>{error.stack}</pre>
          )}
...
try {
  ...
  setTimeout(() => {
    ...
    const appContent = rootElement.innerText.trim();
