...
function useClock() {
    ...
    (0, react_1.useEffect)(function () {
        var tick = function () { return setNow(new Date()); };
        var align = function () {
            var d = new Date();
            var ms = (60 - d.getSeconds()) * 1000 - d.getMilliseconds();
            var t0 = setTimeout(function () {
                tick();
                var id = setInterval(tick, 60000);
                window.__zClock = id;
            }, Math.max(0, ms));
            return function () { return clearTimeout(t0); };
        };
        var cleanup = align();
        return function () {
            cleanup();
            if (window.__zClock)
                clearInterval(window.__zClock);
        };
    }, []);
    return (0, react_1.useMemo)(function () {
        ...
        return "".concat(day, ", ").concat(mon, " ").concat(date, " ").concat(time);
    }, [now]);
}
...
function Header() {
    ...
    return (<header className="zeb-header" role="banner">
      <div className="zeb-header-left">
        <button ref={btnRef} className="zeb-logo-btn" aria-haspopup="menu" aria-expanded={menuOpen} aria-label="Open user menu" onClick={function () { return setMenuOpen(function (v) { return !v; }); }}>
      ...
      <div className="zeb-header-right">
        <button className="zeb-icon-btn" onClick={function () { return navigate("/core"); }} title="Zebulon Core — Memory Database Access (⌘/Ctrl + M)" aria-label="Open Zebulon Core">
          <DatabaseIcon /><span className="zeb-icon-label">Core</span>
        </button>
        <button className="zeb-icon-btn" onClick={function () { return navigate("/settings"); }} title="System Settings (⌘/Ctrl + ,)" aria-label="Open Settings">
          <SettingsIcon /><span className="zeb-icon-label">Settings</span>
        </button>
      </div>

      {menuOpen && (<div className="zeb-user-menu" role="menu" ref={menuRef} aria-label="User menu">
          <button role="menuitem" onClick={function () { navigate("/"); setMenuOpen(false); }}>Dashboard</button>
          <button role="menuitem" onClick={function () { navigate("/zed"); setMenuOpen(false); }}>Zed Lite</button>
          <button role="menuitem" onClick={function () { navigate("/core"); setMenuOpen(false); }}>Zebulon Core</button>
          <button role="menuitem" onClick={function () { navigate("/settings"); setMenuOpen(false); }}>Settings</button>
          <hr />
          <button role="menuitem" onClick={function () { navigate("/about"); setMenuOpen(false); }}>About</button>
        </div>)}
    </header>);
}
