...
async function main() {
    ...
    const port = await (0, get_port_1.default)({ port: Array.from({ length: 127 }, (_, i) => 5173 + i) });
    process.env.VITE_PORT = port.toString();
    // Start the dev server
    const dev = (0, child_process_1.spawn)('pnpm', ['--filter', './client', 'dev'], {
    ...
    const test = (0, child_process_1.spawn)('npx', ['playwright', 'test', `--base-url=http://localhost:${port}`], {
        stdio: 'inherit',
    });
    test.on('exit', (code) => {
        process.exit(code ?? 1);
