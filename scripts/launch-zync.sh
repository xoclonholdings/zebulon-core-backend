#!/bin/bash
# Robust tile launcher for Zync
BACKEND_SCRIPT=./scripts/zync-dev.sh
BACKEND_HEALTH_URL="http://localhost:5500/health"
...
function wait_for_frontend() {
  ...
  pnpm --filter ./apps/zync/client dev &
  for i in {1..14}; do
    sleep 0.5
