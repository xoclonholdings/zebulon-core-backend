#!/bin/bash
# Robust tile launcher for Zlab
BACKEND_SCRIPT=./scripts/zlab-dev.sh
BACKEND_HEALTH_URL="http://localhost:5300/health"
...
function wait_for_frontend() {
  ...
  pnpm --filter ./apps/zlab/client dev &
  for i in {1..14}; do
    sleep 0.5
