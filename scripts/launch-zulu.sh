#!/bin/bash
# Robust tile launcher for Zulu
BACKEND_SCRIPT=./scripts/zulu-dev.sh
BACKEND_HEALTH_URL="http://localhost:5400/health"
...
function wait_for_frontend() {
  ...
  pnpm --filter ./apps/zulu/client dev &
  for i in {1..14}; do
    sleep 0.5
